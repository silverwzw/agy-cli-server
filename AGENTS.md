...
- `build-data/`: Production application assets and source code packaged into the container:
  ...
  - `package.json`, `package-lock.json`: Production Node.js runtime dependencies (Express, Socket.IO, node-pty, posix, etc.).
  ...
  - `client/`: Browser-side frontend assets:
    ...
    - `viewer.html`, `viewer.js`: Web page and client logic for text file viewing with line numbers, interactive highlight selection, and jump-to-line support.
  - `server/`: Server-side Node.js / Express application:
    ...
    - `viewer.js`: File viewer handler providing text file sniffing, size guards, and viewer page rendering.
